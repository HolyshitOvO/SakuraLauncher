                ...
                actions.ForEach(wp =>
                {
                    if (!wp.IsUwpItem)
                    {
                        try
                        {
                            ...
                            //wp.Icon = SystemIcon.ToBitmapImage(WindowsThumbnailProvider.GetThumbnail(wp.targetFilePath, 48, 48, ThumbnailOptions.None));
                        ...
                        {
                            //wp.Icon = SystemIcon.ToBitmapImage(SystemIcon.GetIcon(wp.targetFilePath, true).ToBitmap());// 这个也可以，但是某些图标不行
