...
namespace RunnerPlugin
{
	...
	public sealed class ListedRunnerPlugin : QuickPlugin
	{
		...
		{
			// 启动图标更新任务
			taskManager.StartTask("UpdateIcons", token =>
			{
				List<BitmapImage> tempIcons = new List<BitmapImage>();

				try
				{
					// 创建副本，避免集合冲突
                    List<RunCommandAction> actions2 = actions.ToList();

                    foreach (var wp in actions2)
					{
						// 及时检查是否任务是否取消
						...
						if (!wp.IsUwpItem)
						{
							try
							{
								tempIcons.Add(SystemIcon.ToBitmapImage(WindowsThumbnailProvider.GetThumbnail(wp.targetFilePath, 48, 48, ThumbnailOptions.None)));
							}
							catch (Exception)
							{
								try
								{
									tempIcons.Add(SystemIcon.ToBitmapImage(SystemIcon.GetIcon(wp.targetFilePath, true).ToBitmap()));
								}
								catch (Exception)
								{
									tempIcons.Add(null);
					...
					if (!token.IsCancellationRequested)
					{
						...
						{
							SynchronizationContext.Current.Send(_ => AddIconToActions(tempIcons), null);
						}
						else
						{
							AddIconToActions(tempIcons);
						}
					}
				}
		...
		private void AddIconToActions(List<BitmapImage> tempIcons)
		{
			for (int i = 0; i < actions.Count; i++)
			{
				if (!actions[i].IsUwpItem)
				{
					actions[i].Icon = tempIcons[i];
				}
			}
		}

		public void CancelUpdateIcon()
		{
			taskManager.CancelTask("UpdateIcons");
		}
